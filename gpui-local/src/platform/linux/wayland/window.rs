...
use wayland_client::{Proxy, protocol::wl_surface, protocol::wl_output};
...
impl WaylandWindowStatePtr {
    ...
    /// Explicitly request a frame from the compositor (for cross-window sync)
    pub fn request_frame_explicit(&self) {
        // Use try_borrow_mut to avoid panic when state is already borrowed
...
impl PlatformWindow for WaylandWindow {
    ...
    fn request_explicit_frame(&self) {
        // Request a frame callback from the compositor even if unfocused
        // self.frame() calls surface.frame() and triggers the render callback
        self.0.frame();
    }
